<template>
    <v-container fluid>
        <v-row>
            <v-col cols="8" style="border-style:solid; border-width: 1px;">
                <v-row>
                    <v-col v-for="(card, index) in even(card_items)" :key="2*index" cols="6">
                        <v-card :title = card.title :text = card.contents >
                            <v-row class="justify-end" style="margin:5px;">
                                <v-icon :icon="mdiPencil" @click="openDialog(2*index)" color="grey"/>
                                <v-icon :icon="mdiContentSave" @click="saveCard(2*index)" color="grey"/>
                            </v-row>
                        </v-card>
                    </v-col>
                </v-row>
            ...
            <v-col cols="4" style="border-style:solid; border-width: 1px;">
                <v-row>
                    <v-col v-for="(card, index) in odd(card_items)" :key="1+(2*index)" cols="12">
                        <v-card :title = card.title :text = card.contents >
                            <v-row class="justify-end" style="margin:5px;">
                                <v-icon :icon="mdiPencil" @click="openDialog(1+(2*index))" color="grey"/>
                                <v-icon :icon="mdiContentSave" @click="saveCard(1+(2*index))" color="grey"/>
                            </v-row>
                        </v-card>
                    </v-col>
                </v-row>
